...
class YOLO_TF:
	...
	filewrite_txt = False
	disp_console = True
	weights_file = 'weights/YOLO_face'
	alpha = 0.1
	threshold = 0.2
	iou_threshold = 0.5
	num_class = 1
	...
	def interpret_output(self,output):
		...
		class_probs = np.reshape(output[0:prob_range[1]],(self.grid_size,self.grid_size,self.num_class))
		scales = np.reshape(output[scales_range[0]:scales_range[1]],(self.grid_size,self.grid_size,self.num_box))
		boxes = np.reshape(output[boxes_range[0]:],(self.grid_size,self.grid_size,self.num_box,4))
		offset = np.transpose(np.reshape(np.array([np.arange(self.grid_size)]*(2*self.grid_size)),(2,self.grid_size,self.grid_size)),(1,2,0))

		boxes[:,:,:,0] += offset
		boxes[:,:,:,1] += np.transpose(offset,(1,0,2))
		boxes[:,:,:,0:2] = boxes[:,:,:,0:2] / float(self.grid_size)
